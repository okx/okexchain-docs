...
- **Application key**: This key is created from `exchaincli` and used to sign transactions. Application keys are associated with a public key prefixed by `okchainpub` and an address prefixed by `okexchain`. Both are derived from account keys generated by `exchaincli keys add`.
