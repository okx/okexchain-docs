...
OKBC needs to parse and handle transactions routed for both the EVM and for the Cosmos hub. We
...
Being that OKBC implements the Tendermint ABCI application interface, as transactions are
...
internal interaction between OKBC and Tendermint will utilize one of the supported encoding
formats: Protobuf and Amino.
...
OKBC supports [EIP-155](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md)
signatures. A `Transaction` is expected to have a single signature for Ethereum routed transactions.
However, just as in Cosmos, OKBC will support multiple signers for non-Ethereum transactions.
