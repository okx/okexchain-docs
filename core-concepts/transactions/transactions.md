...
Being that OKTC implements the Tendermint ABCI application interface, as transactions are
...
internal interaction between OKTC and Tendermint will utilize one of the supported encoding
...
OKTC supports [EIP-155](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-155.md)
signatures. A `Transaction` is expected to have a single signature for Ethereum routed transactions.
However, just as in Cosmos, OKTC will support multiple signers for non-Ethereum transactions.
