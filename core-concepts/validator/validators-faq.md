...
- **Tendermint Key**: This is a unique key used to sign consensus votes.
  - It is associated with a public key `oktchainvalconspub` (Get this value with `exchaind tendermint show-validator`)
  - It is generated when the node is created with exchaind init.
- **Application key**: This key is created from `exchaincli` and used to sign transactions. Application keys are associated with a public key prefixed by `oktchainpub` and an address prefixed by `oktc`. Both are derived from account keys generated by `exchaincli keys add`.

Note: A validator’s operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `oktchainvaloper` and `oktchainvaloperpub`.
