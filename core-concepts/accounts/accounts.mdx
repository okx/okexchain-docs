...
This document describes the in-built accounts system of OKTC.

## Pre-requisite readings

- [Cosmos SDK Accounts](https://docs.cosmos.network/v0.46/basics/accounts)
- [Ethereum Accounts](https://ethereum.org/en/whitepaper/#ethereum-accounts)

## OKTC accounts

OKTC defines its own custom `Account` type that uses Ethereum’s ECDSA secp256k1 curve for keys. This
satisfies the [EIP84](https://github.com/ethereum/EIPs/issues/84) for full [BIP44](https://github.com/bitcoin/bips/blob/master/bip-0044.mediawiki) paths.
The root HD path for OKTC-based accounts is `m/44'/60'/0'/0`.

## Addresses and public keys

There are 3 main types of `Addresses`/`PubKeys` available by default on OKTC:
...
`OKTCAccount`s can be represented in both [Bech32](https://en.bitcoin.it/wiki/Bech32) and hex format for Ethereum's Web3 tooling compatibility.
...
You can query an account address using the OKTC CLI or REST clients:
...
<Tip>
  The Cosmos SDK Keyring output (i.e `exchaincli keys`) only supports addresses
  and public keys in Bech32 format.
</Tip>

To retrieve the Ethereum hex address using Web3, use the JSON-RPC [`oktc_accounts`](/api/oktc-api/json-rpc-api#eth_accounts) endpoint:
