...
To deploy your new mydapp smart contract on OKBC testnet, run the following command in the terminal with extral flag `--network testnet`.

```sh
 bytecraft deploy mydapp --signer test --network okbc-testnet
```
...
The network is specified with `testnet`. your  contract will be deployed to OKBC `testnet`

### Deploying on OKBC Testnet
...
```sh
bytecraft console --signer test --network okbc-testnet
...
```sh
bytecraft task:run template --signer test --network okbc-testnet
```
...
Now instead of running `bytecraft task:run deploy_counter --signer test --network okbc-testnet` you can run `bytecraft deploy mydapp --signer test --network testnet`.

## Use Bytecraft with OKBC Mainnet

> OKBC Mainnet coming soon
...
To deploy your new my-wasm-dapp smart contract on OKBC mainnet, run the following command in the terminal with extral flag `--network mainnet`.

```
 bytecraft deploy mydapp --signer test --network okbc-mainnet
```

In this case, `test`, as our signer. The signer account will be responsible for paying the gas fee associated with deploying the contract to the exchain blockchain and will be assigned as the owner of the project.

The network is specified with `mainnet`. your  contract will be deployed to OKBC `mainnet`

### Deploying on OKBC Mainnet
...
```
bytecraft console --signer test --network okbc-mainnet
bytecraft > lib.freeze()
#output
{
  logs: [ { msg_index: 0, log: '', events: [Array] } ],
...
```
bytecraft task:run template --signer test --network okbc-mainnet
```
...
Now instead of running `bytecraft task:run deploy_counter --signer test --network okbc-mainnet` you can run `bytecraft deploy mydapp --signer test --network mainnet`.
